    ...
    <div style={{ backgroundColor: 'white', borderRadius: '14px', border: `2px solid ${topic.launchScore >= 70 ? launchColor.border : '#e5e5e5'}`, padding: '16px', boxShadow: topic.launchScore >= 85 ? '0 4px 20px rgba(220, 38, 38, 0.15)' : 'none' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '12px', backgroundColor: launchColor.bg, borderRadius: '10px', border: `1px solid ${launchColor.border}` }}>
        ...
        <div style={{ textAlign: 'right' }}>
          ...
          {topic.crossPlatform && <div style={{ fontSize: '9px', color: '#16a34a', marginTop: '4px', fontWeight: '600' }}>✓ Multi-source</div>}
        </div>
      </div>
...
export default function App() {
  ...
  const [trendSource, setTrendSource] = useState({ hackernews: false, lemmy: false, crypto: false, dex: false });
      ...
      if (res.ok) {
        try {
          ...
          if (data.success && data.topics?.length) {
            ...
            const d = data.debug || {};
            setDebugInfo(`✓ Loaded ${data.topics.length} topics | HN: ${d.hackernews || 0} | Lemmy: ${d.lemmy || 0} | Crypto: ${d.crypto || 0} | DEX: ${d.dex || 0}`);
          } else {
      ...
      <div style={{ maxWidth: '1200px', margin: '0 auto' }}>
        <div style={{ textAlign: 'center', marginBottom: '20px' }}>
          ...
          <p style={{ fontSize: '12px', color: '#737373', margin: '0 0 10px' }}>
            {activeTab === 'launch' ? 'Real-time trend detection from multiple sources' : activeTab === 'memes' ? 'Live from DexScreener' : activeTab === 'markets' ? 'Live from Polymarket' : 'Live prices'}
          </p>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px', gap: '12px', flexWrap: 'wrap' }}>
          ...
          {activeTab === 'launch' && (
            <div style={{ display: 'flex', gap: '6px' }}>
              {[['all', 'All'], ['hot', '🔥 Hot'], ['crypto', '🪙 Crypto'], ['tech', '💻 Tech']].map(([key, label]) => (
        ...
        {activeTab === 'launch' && (
          <div>
            <div style={{ backgroundColor: '#f0fdf4', border: '1px solid #bbf7d0', borderRadius: '12px', padding: '14px 18px', marginBottom: '16px' }}>
              <p style={{ margin: 0, fontSize: '12px', color: '#166534' }}><strong>🔍 How it works:</strong> Aggregates trends from Hacker News (tech), Lemmy (community), CoinGecko (crypto), and GeckoTerminal DEX (meme coins). Multi-source topics get boosted scores!</p>
            </div>
