...
export default function App() {
  ...
  const fetchTrends = useCallback(async () => {
    ...
    try {
      const res = await fetch('/api/aggregate', { signal: AbortSignal.timeout(25000) });
      if (res.ok) {
        const data = await res.json();
        if (data.success && data.topics?.length) {
          setTrends(data.topics);
          setTrendSource(data.sources);
      ...
      if (googleRes.status === 'fulfilled' && googleRes.value?.trends) {
        hasGoogle = true;
      ...
      if (redditRes.status === 'fulfilled' && redditRes.value?.topics) {
        hasReddit = true;
      ...
      if (!hasGoogle && !hasReddit) setError('APIs may be rate limited. Deploy to Vercel for full functionality.');
    } catch (err) {
      setError('Deploy to Vercel for live trend data. Local dev has CORS limitations.');
    } finally {
      setTrendLoading(false);
    }
  }, []);
        ...
        {activeTab === 'launch' && (
          <div>
            ...
            {trendLoading && !trends.length ? (
              ...
              <div style={{ padding: '60px', textAlign: 'center', color: '#737373' }}><div style={{ fontSize: '32px', marginBottom: '12px' }}>🔍</div>Deploy to Vercel to enable live trend APIs</div>
            ) : (
