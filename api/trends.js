// Vercel Serverless Function - Fetches Google Trends data
export default async function handler(req, res) {
  ...
  try {
    ...
    for (const day of (data.default?.trendingSearchesDays || []).slice(0, 2)) {
      for (const search of (day.trendingSearches || [])) {
        ...
        trends.push({
          ...
          articles: (search.articles || []).slice(0, 2).map(a => ({ title: a.title, source: a.source, url: a.url, image: a.image?.imageUrl })),
    ...
    res.status(200).json({ success: true, timestamp: new Date().toISOString(), source: 'google_trends', geo, trends: trends.slice(0, 25) });
  } catch (error) {
